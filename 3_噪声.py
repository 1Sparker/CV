...
功能：为浮点型图片添加各种随机噪声
参数： 
image：输入图片(将会被转化为浮点型)，ndarray型
mode: 可选择，str型，表示要添加的噪声类型
     gaussian: 高斯噪声
     localvar: 高斯分布的加性噪声，在“图像” 的每个点处具有指定的局部方差
     poisson: 泊松噪声
     salt: 盐噪声，随机将像素值变成1
     pepper: 椒噪声，随即将像素值变成0或-1，取决于矩阵的值是否带符号
     s&p：椒盐噪声
 ...
 salt_vs_pepper: 可选的，float型，椒盐噪声中椒盐的比例，值越大表示盐噪声越多，默认值 = 0.5，即椒盐等量
 --------
...
# cv.imshow("lenna vs noise png",np.hstack((img, noise_gs_img)))
