...
(train_images, train_labels),(test_images, test_labels) = mnist.load_data()  #网络下载MNIST数据集
...
#model.compile()方法用于在配置训练方法时，告知训练时用的优化器、损失函数和准确率评测标准  model.compile(optimizer = 优化器，loss = 损失函数，metrics = ["准确率”])
...
to_categorical(y,num_classes = None)[y: 类别向量,num_classes:总共类别数]将类别向量(从0到nb_classes的整数向量)映射为二值类别矩阵, 用于应用到以categorical_crossentropy为目标函数的模型中.
'''
...
#verbose：日志显示，verbose = 0 为不在标准输出流输出日志信息；= 1 为输出进度条记录； = 2 没有进度条，只是输出一行记录
print(test_loss)
